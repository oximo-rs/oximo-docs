...
/// Native indicators can be explicitly reformulated for backends such as HiGHS.
fn indicator_constraints() -> Result<(), Box<dyn std::error::Error>> {
    println!("== indicator constraints ==");
    ...
    let result = Highs.solve(&m, &HighsOptions::default())?;
    println!(
        "objective = {:?}, reformulated indicators = {}\n",
        result.objective(),
        artifacts.len()
    );
    Ok(())
...
/// Native SOS constraints can be explicitly reformulated for HiGHS.
fn sos_constraints() -> Result<(), Box<dyn std::error::Error>> {
    println!("== SOS constraints ==");
    ...
    let result = Highs.solve(&m, &HighsOptions::default())?;
    println!(
        "objective = {:?}, reformulated SOS constraints = {}\n",
        result.objective(),
        artifacts.len()
    );
    Ok(())
